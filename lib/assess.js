...
async function searchWeb(query, searchLog) {
  ...
  // 第一层：axios
  try {
    html = await searchWithAxios(query);
    method = 'axios';
  } catch (err) {
    errorMsg = `axios: ${err.message}`;
  }

  // 如果 axios 返回空或太短，尝试 curl 备选
  if (!html || html.length < 500) {
    searchLog.push({
      ...
      source: 'Bing 搜索引擎',
      query: query,
      message: `axios 返回 ${html ? html.length : 0} 字节，尝试 curl 备选...`,
      error: errorMsg || undefined
    });
    try {
      html = await searchWithCurl(query);
      method = 'curl';
    } catch (err2) {
      errorMsg += `; curl: ${err2.message}`;
    }
  }
  ...
  if (hasResults) {
    searchLog.push({
      ...
      textLength: text.length,
      message: `Bing 搜索成功（通过 ${method}），返回 ${html.length} 字节 HTML，提取 ${text.length} 字符有效文本`
    });
  } else if (html.length > 0) {
    // 有 HTML 但无法提取有效内容（可能是验证码页面）
    ...
    searchLog.push({
      ...
      textLength: text.length,
      htmlPreview: preview,
      message: `Bing 返回 ${html.length} 字节 HTML 但未能提取有效搜索结果（可能是反爬虫拦截）`,
      error: errorMsg || undefined
    });
  } else {
    searchLog.push({
...
function extractSearchText(html) {
  ...
  // 如果内容明显是验证码/拦截页面，直接返回空
  const lower = html.toLowerCase();
  if (lower.includes('captcha') || lower.includes('verify') || lower.includes('人机验证') ||
      lower.includes('are you a robot') || lower.includes('unusual traffic') ||
      lower.includes('请稍后') || lower.includes('正在验证')) {
    return '';
  }
...
async function searchDrivenAssess(companyName) {
  ...
  // 第一优先级：企查查 API
  if (qccClient) {
    const qccResult = await qccClient.assessByQCC(companyName, searchLog);
    if (qccResult && qccResult.checkedItems) {
  ...
  // 第二优先级：启信宝 API
  if (qixinClient) {
    const qixinResult = await qixinClient.assessByQixin(companyName, searchLog);
    if (qixinResult && qixinResult.checkedItems) {
  ...
  // 第三优先级：搜索引擎（Bing / Brave）
  searchLog.push({
    step: 'web_search',
    status: 'running',
    source: '搜索引擎',
    message: '企查查/启信宝均不可用，尝试通过搜索引擎获取信息...'
  });
